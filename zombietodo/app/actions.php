...
switch ($action) { // switch on kuin monta if-else:ä peräkkäin — siistimpi tapa
    ...
    case 'set_operation':    handleSetOperation();    break; // Vaihdetaan aktiivinen operaatio (sessioon)
    case 'add_operation':    handleAddOperation();    break; // Luodaan uusi operaatio
    case 'edit_operation':   handleEditOperation();   break; // Muokataan olemassa olevaa operaatiota
    case 'delete_operation': handleDeleteOperation(); break; // Poistetaan tyhjä operaatio
...
// ===========================================================
// APUFUNKTIO — VARMISTA ETTÄ OPERAATIO KUULUU KÄYTTÄJÄLLE
// Tarkistaa AINA kannasta suoraan, ei luoteta annettuun arvoon sellaisenaan.
// Palauttaa operaation rivin (id, name, description, color) tai null.
// ===========================================================
function op_findOwned($conn, $operationId, $userId) {
    $operationId = intval($operationId);
    if ($operationId <= 0) return null;
    $stmt = $conn->prepare('SELECT id, name, description, color FROM operations WHERE id=? AND user_id=?');
    $stmt->bind_param('ii', $operationId, $userId);
    $stmt->execute();
    $row = $stmt->get_result()->fetch_assoc();
    $stmt->close();
    return $row ?: null;
...
// Validoi hex-väri muotoa #rrggbb. Palauttaa siistityn värin tai null jos virheellinen/tyhjä.
function op_validColor($raw) {
    $raw = trim((string)$raw);
    if ($raw === '') return null;
    if (!preg_match('/^#[0-9a-fA-F]{6}$/', $raw)) return null;
    return strtolower($raw);
}

// ===========================================================
// OPERAATIOTOIMINNOT — vain kirjautuneille käyttäjille
// Operaatiot ovat "projekteja" joiden alle tehtävät ryhmitellään.
// ===========================================================

// Vaihdetaan istunnon aktiivinen operaatio
function handleSetOperation() {
    ...
    // Omistajuustarkistus AINA kannasta — POST-arvoon ei luoteta sellaisenaan
    $op = op_findOwned($conn, $operationId, $userId);
    if (!$op) {
        echo json_encode(['success' => false, 'error' => 'Operaatiota ei löydy.']);
        exit;
    }

    $_SESSION['active_operation'] = (int)$op['id'];

    // Muistetaan valinta kantaan yli uloskirjautumisen — käytetään aktiivisena seuraavalla kirjautumisella
    $lastOpId = (int)$op['id'];
    $stmt = $conn->prepare('UPDATE users SET last_operation_id=? WHERE id=?');
    $stmt->bind_param('ii', $lastOpId, $userId);
    $stmt->execute();
    $stmt->close();
...
function handleAddOperation() {
    ...
    if ($name === '') {
        echo json_encode(['success' => false, 'error' => 'Operaation nimi ei voi olla tyhjä.']);
        exit;
    }
    if (mb_strlen($name) > 100) {
        echo json_encode(['success' => false, 'error' => 'Nimi saa olla enintään 100 merkkiä.']);
        exit;
    }
    if (mb_strlen($description) > 255) {
        echo json_encode(['success' => false, 'error' => 'Kuvaus saa olla enintään 255 merkkiä.']);
        exit;
    }
    // Väri validoidaan tiukasti — vain #rrggbb hyväksytään, muuten tallennetaan NULL
    $color = op_validColor($colorRaw);
    if ($colorRaw !== '' && $color === null) {
        echo json_encode(['success' => false, 'error' => 'Virheellinen väri.']);
        exit;
    }
    $descriptionVal = $description !== '' ? $description : null;

    $stmt = $conn->prepare('INSERT INTO operations (user_id, name, description, color) VALUES (?, ?, ?, ?)');
    $stmt->bind_param('isss', $userId, $name, $descriptionVal, $color);
    $stmt->execute();
    $newId = $conn->insert_id;
    $stmt->close();

    // Uusi operaatio otetaan heti aktiiviseksi — muistetaan myös kantaan yli uloskirjautumisen
    $_SESSION['active_operation'] = $newId;
    $lastOpStmt = $conn->prepare('UPDATE users SET last_operation_id=? WHERE id=?');
    $lastOpStmt->bind_param('ii', $newId, $userId);
    $lastOpStmt->execute();
    $lastOpStmt->close();

    echo json_encode(['success' => true, 'operation' => [
        'id' => $newId, 'name' => $name, 'description' => $description, 'color' => $color
    ]]);
    exit;
}

// Muokataan olemassa olevaa operaatiota
function handleEditOperation() {
    ...
    $userId      = intval($_SESSION['user_id']);
    $operationId = intval($_POST['id'] ?? 0);
    $name        = trim($_POST['name'] ?? '');
    $description = trim($_POST['description'] ?? '');
    $colorRaw    = $_POST['color'] ?? '';

    // Omistajuustarkistus AINA kannasta ENNEN muokkausta
    $op = op_findOwned($conn, $operationId, $userId);
    if (!$op) {
        echo json_encode(['success' => false, 'error' => 'Operaatiota ei löydy.']);
        exit;
    ...
    $stmt = $conn->prepare('UPDATE operations SET name=?, description=?, color=? WHERE id=? AND user_id=?');
    $stmt->bind_param('sssii', $name, $descriptionVal, $color, $operationId, $userId);
    $stmt->execute();
    $stmt->close();

    echo json_encode(['success' => true, 'operation' => [
        'id' => $operationId, 'name' => $name, 'description' => $description, 'color' => $color
    ]]);
    exit;
}

// Poistetaan operaatio — sallittu vain jos siinä ei ole yhtään tehtävää
function handleDeleteOperation() {
    ...
    // Omistajuustarkistus AINA kannasta ENNEN poistoa
    $op = op_findOwned($conn, $operationId, $userId);
    if (!$op) {
        echo json_encode(['success' => false, 'error' => 'Operaatiota ei löydy.']);
        exit;
    ...
    // Sallitaan poisto vain jos operaatiossa ei ole yhtään tehtävää
    $stmt = $conn->prepare('SELECT COUNT(*) AS cnt FROM tasks WHERE operation_id=? AND user_id=?');
    $stmt->bind_param('ii', $operationId, $userId);
    $stmt->execute();
    $taskCount = intval($stmt->get_result()->fetch_assoc()['cnt']);
    $stmt->close();

    if ($taskCount > 0) {
        echo json_encode(['success' => false, 'error' => 'Operaatiossa on tehtäviä — tyhjennä se ensin']);
        exit;
    }

    $stmt = $conn->prepare('DELETE FROM operations WHERE id=? AND user_id=?');
    $stmt->bind_param('ii', $operationId, $userId);
    $stmt->execute();
    $stmt->close();

    // Jos poistettu operaatio oli aktiivinen, nollataan sessio — tasks.php valitsee seuraavan
    if (!empty($_SESSION['active_operation']) && (int)$_SESSION['active_operation'] === $operationId) {
        unset($_SESSION['active_operation']);
    }
...
function handleTaskAction($action) {
    ...
    if ($action === 'add') {
        ...
        // Tehtävä tallennetaan istunnon aktiiviseen operaatioon. Operaatio tarkistetaan
        // AINA kannasta — sessioarvoon ei luoteta sellaisenaan.
        $activeOperationId = intval($_SESSION['active_operation'] ?? 0);
        $op = op_findOwned($conn, $activeOperationId, $user_id);
        if (!$op) {
            echo json_encode(['success' => false, 'error' => 'Valitse tai luo operaatio ensin.']);
            exit;
        }
        $operationId = (int)$op['id'];

        $stmt = $conn->prepare("INSERT INTO tasks (user_id, operation_id, text, status, created_at) VALUES (?, ?, ?, 'not_started', NOW())");
        $stmt->bind_param('iis', $user_id, $operationId, $task); // 'iis' = integer, integer, string
        $stmt->execute();
        $stmt->close();
        echo json_encode(['success' => true]);
        exit;
    ...
    if ($action === 'get_task' && $id > 0) {
        $stmt = $conn->prepare('SELECT id, text, hours, parent_id, status, started_at, done_at FROM tasks WHERE id=? AND user_id=?');
    ...
    if ($action === 'edit_task' && $id > 0) {
        ...
        $hoursRaw   = trim($_POST['hours']      ?? ''); // Tunnit — pilkku sallitaan desimaalierottimena
        $parentRaw  = $_POST['parent_id'] ?? '';        // Päätehtävän valinta alasvetovalikosta (tyhjä = ei muutosta pois)

        // Haetaan muokattavan tehtävän oma operaatio — myös vahvistaa omistajuuden.
        // Päätehtävä pitää löytyä samasta operaatiosta.
        $ownChk = $conn->prepare('SELECT operation_id FROM tasks WHERE id=? AND user_id=?');
        $ownChk->bind_param('ii', $id, $user_id);
        $ownChk->execute();
        $ownRow = $ownChk->get_result()->fetch_assoc();
        $ownChk->close();
        if (!$ownRow) {
            echo json_encode(['success' => false, 'error' => 'Tehtävää ei löydy']);
            exit;
        }
        $taskOperationId = $ownRow['operation_id'] !== null ? (int)$ownRow['operation_id'] : null;
        ...
        // Käsitellään tunnit — tyhjä = 0, pilkku pisteeksi
        $hoursRaw = str_replace(',', '.', $hoursRaw);
        if ($hoursRaw === '') {
            $hours = 0.0;
        } elseif (is_numeric($hoursRaw)) {
            $hours = (float)$hoursRaw;
        } else {
            echo json_encode(['success' => false, 'error' => 'Tunnit pitää olla numero.']);
            exit;
        }
        if ($hours < 0 || $hours > 999.99) {
            echo json_encode(['success' => false, 'error' => 'Tunnit pitää olla välillä 0–999,99.']);
            exit;
        }

        // Käsitellään päätehtävän valinta — 0 = päätehtävä (NULL)
        $parent_id = intval($parentRaw);
        if ($parent_id === $id) {
            echo json_encode(['success' => false, 'error' => 'Tehtävä ei voi olla oma päätehtävänsä.']);
            exit;
        }
        if ($parent_id > 0) {
            // Varmistetaan että valittu päätehtävä on olemassa, kuuluu käyttäjälle
            // JA on samassa operaatiossa kuin muokattava tehtävä — ei ristiin operaatioiden.
            $chk = $conn->prepare('SELECT id FROM tasks WHERE id=? AND user_id=? AND operation_id <=> ?');
            $chk->bind_param('iii', $parent_id, $user_id, $taskOperationId);
            $chk->execute();
            $exists = $chk->get_result()->num_rows > 0;
            $chk->close();
            if (!$exists) {
                echo json_encode(['success' => false, 'error' => 'Valittua päätehtävää ei löydy.']);
                exit;
            }
            // Estetään kiertosilmukka: kuljetaan valitun päätehtävän ketjua ylöspäin.
            // Jos matkalla tulee vastaan muokattava tehtävä itse, syntyisi silmukka (esim. A→B→C→A),
            // joka rikkoisi tuntisummauksen (ääretön rekursio). Torjutaan se.
            $ancestor = $parent_id;
            $safety = 0; // Turvaraja ettei jää ikuiseen luuppiin jos data on jo valmiiksi rikki
            while ($ancestor !== null && $safety < 1000) {
                if ((int)$ancestor === $id) {
                    echo json_encode(['success' => false, 'error' => 'Tehtävää ei voi asettaa oman alitehtävänsä alle.']);
                    exit;
                }
                // Haetaan seuraavan tason ylätehtävä
                $up = $conn->prepare('SELECT parent_id FROM tasks WHERE id=? AND user_id=?');
                $up->bind_param('ii', $ancestor, $user_id);
                $up->execute();
                $row = $up->get_result()->fetch_assoc();
                $up->close();
                $ancestor = ($row && $row['parent_id'] !== null) ? (int)$row['parent_id'] : null;
                $safety++;
            }
            $parentVal = $parent_id;
        } else {
            $parentVal = null; // Päätehtävä
        }
        ...
        $stmt = $conn->prepare('UPDATE tasks SET text=?, hours=?, parent_id=?, started_at=?, done_at=?, status=? WHERE id=? AND user_id=?');
        $stmt->bind_param('sdisssii', $text, $hours, $parentVal, $started_at, $done_at, $status, $id, $user_id); // 'd'=tunnit, parent_id voi olla NULL
        $stmt->execute();
        $stmt->close();
        echo json_encode(['success' => true]);
        exit;
    ...
    // ===========================================================
    // KOOSTE — palautetaan kaikki käyttäjän tehtävät JSON-listana
    // koostemodalia varten (tuntikooste + aikajana rakennetaan JS:ssä).
    // ===========================================================
    if ($action === 'summary') {
        // Kooste rajataan istunnon aktiiviseen operaatioon. Operaatio tarkistetaan
        // AINA kannasta — sessioarvoon ei luoteta sellaisenaan.
        $activeOperationId = intval($_SESSION['active_operation'] ?? 0);
        $op = op_findOwned($conn, $activeOperationId, $user_id);
        if (!$op) {
            echo json_encode(['success' => true, 'tasks' => []]); // Ei aktiivista operaatiota — tyhjä kooste
            exit;
        }
        $operationId = (int)$op['id'];

        $stmt = $conn->prepare('SELECT id, text, parent_id, hours, status, started_at, done_at, created_at FROM tasks WHERE user_id=? AND operation_id=? ORDER BY id ASC');
        $stmt->bind_param('ii', $user_id, $operationId);
        $stmt->execute();
        $res = $stmt->get_result();
        $tasks = [];
