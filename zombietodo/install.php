...
CREATE TABLE IF NOT EXISTS users (
    ...
    last_operation_id INT NULL,                 -- Viimeksi valittu operaatio — muistetaan yli uloskirjautumisen. FK lisätään erikseen alla, koska operations-taulua ei ole vielä tässä vaiheessa.
...
// Operaatiotaulu — käyttäjän projektit ("operaatiot") joiden alle tehtävät ryhmitellään
$conn->query("
CREATE TABLE IF NOT EXISTS operations (
    id INT AUTO_INCREMENT PRIMARY KEY,          -- Jokaiselle operaatiolle oma numero, kasvaa automaattisesti
    user_id INT NOT NULL,                       -- Kenen operaatio on, pakollinen
    name VARCHAR(100) NOT NULL,                 -- Operaation nimi, maksimissaan 100 merkkiä, pakollinen
    description VARCHAR(255) NULL,              -- Vapaaehtoinen lyhyt kuvaus, maksimissaan 255 merkkiä
    color VARCHAR(7) NULL,                      -- Vapaaehtoinen tunnusväri hex-muodossa, esim. #cc0000
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, -- Milloin operaatio luotiin, täyttyy automaattisesti
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE -- Jos käyttäjä poistetaan, poistetaan myös hänen operaationsa
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
");

// users.last_operation_id -> operations(id) lisätään vasta nyt, koska operations-taulua
// ei ollut vielä olemassa kun users luotiin. ON DELETE SET NULL — jos viimeksi valittu
// operaatio poistetaan, kenttä vain nollautuu eikä mikään kaadu.
$conn->query("
ALTER TABLE users
    ADD FOREIGN KEY (last_operation_id) REFERENCES operations(id) ON DELETE SET NULL
");
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    operation_id INT NULL,                      -- Mihin operaatioon (projektiin) tehtävä kuuluu
    text VARCHAR(255) NOT NULL,                 -- Tehtävän teksti, maksimissaan 255 merkkiä, pakollinen
    parent_id INT NULL,                         -- Päätehtävän id — NULL = päätehtävä. Alitehtävällä voi olla omia alitehtäviä
    hours DECIMAL(5,2) NOT NULL DEFAULT 0,      -- Tehtävälle kirjatut tunnit, syötetään muokkausmodalissa. Enintään 999.99
    ...
    FOREIGN KEY (operation_id) REFERENCES operations(id) ON DELETE CASCADE, -- Jos operaatio poistetaan, poistetaan myös sen tehtävät (poisto sallitaan sovelluksessa vain tyhjälle operaatiolle)
    FOREIGN KEY (parent_id) REFERENCES tasks(id) ON DELETE SET NULL -- Jos päätehtävä poistetaan, alitehtävistä tulee itsenäisiä (parent_id nollataan), ei poisteta
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4
");
